...
anonymity changes what a cached row may contain. Built so far: server
skeleton + JWT-forwarding (2026-08-08), the `feed_shared` RPC (2026-08-10),
`GET /api/feed` (2026-08-20), and — **complete as of 2026-08-27** — Redis
caching on that endpoint (read-through, single-flight dedup on the miss path,
and full test coverage). The gateway/rate-limit decisions below are resolved
but not yet built, and no client hook has been wired to the new server yet.
Build log: `[[project-phase-status]]`.
